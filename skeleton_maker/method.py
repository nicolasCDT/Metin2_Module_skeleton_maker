...
def format_snake_case(text: AnyStr) -> AnyStr:
	"""Return a varname in snake case style"""
	return '_'.join(
		re.sub('([A-Z][a-z]+)', r' \1', re.sub('([A-Z]+)', r' \1', text.replace('-', ' '))).split()).lower()
...
class Method:
	...
	def __init__(self: me, py_name: AnyStr, cpp_name: AnyStr, content: AnyStr) -> None:
		...
		self._args_raw_name: List[AnyStr] = list()
		...
		for cpp_function, py_type in FUNCTIONS_TYPE_CORRESPONDENCES.items():
			...
			if raw_args:
				for i, name in raw_args:
					name = name.replace(".", "_")  # Replace . by _
					if FORMAT_ARGUMENTS_TO_SNAKE_CASE:
						name = format_snake_case(name)
					new_name = self.get_available_name_for(name)  # Get first free name
					self._args[int(i)] = Argument(new_name, py_type)  # Create argument
					self._args_raw_name.append(new_name)  # Add this name to list
	...
	def get_available_name_for(self: me, name: AnyStr) -> AnyStr:
		"""Returns a free name for argument"""
		if name not in self._args_raw_name:
			return name

		i: int = 0
		while f"{name}{i}" in self._args_raw_name:
			i += 1
		return f"{name}{i}"

	def __str__(self: me) -> AnyStr:
		"""String representation"""
		return f"({self._cpp_name}|{self._py_name})"

	def __repr__(self: me) -> AnyStr:
		"""String representation"""
		return f"({self._cpp_name}|{self._py_name})"
